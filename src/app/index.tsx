...
import {green, bold, cyan} from 'ansis';
...
export const ZeugApp = new Elysia({})
    ...
    .get("/i/:id/", async ({params: {id}, itemRepository}) => {
    ...
    .get('/i/:id/edit', async ({params: {id}, itemRepository}) => {
        const item = await itemRepository.findById(id);
        if (!item) {
            return <Layout title={`Item ${id}`}>Sorry, nothing to edit.</Layout>;
        }
        return <Layout title={`Item ${id}`}><ItemEdit item={item}/></Layout>;
    }, {
        params: t.Object({
            id: t.Number()
        })
