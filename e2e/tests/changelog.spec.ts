...
test.describe("Changelog / Audit Log", () => {
  ...
  test("should display changelog entries", async ({ authenticatedPage: page }) => {
    await page.goto("/changelog")
  ...
  test("should show change type badges", async ({ authenticatedPage: page }) => {
    await page.goto("/changelog")
  ...
  test("should show user who made the change", async ({ authenticatedPage: page }) => {
    await page.goto("/changelog")
  ...
  test("should show timestamp of changes", async ({ authenticatedPage: page }) => {
    await page.goto("/changelog")
  ...
  test("should show total entries count", async ({ authenticatedPage: page }) => {
    await page.goto("/changelog")

    // Look for "X Einträge insgesamt" text
    await expect(page.locator('text=/\\d+ Einträge? insgesamt/')).toBeVisible({ timeout: 5000 })
  })

  test("should expand entry to show diff view", async ({ authenticatedPage: page }) => {
    await page.goto("/changelog")
    ...
    await expect(diffView).toBeVisible({ timeout: 3000 }).catch(() => {
      // It's okay if diff view doesn't show - entry might not be expandable
    })
  })

  test("should show changed fields in diff", async ({ authenticatedPage: page }) => {
    await page.goto("/changelog")
    ...
    const fieldLabels = page.locator('text=/Name|Beschreibung|Kategorie|Erstellte Werte|Geänderte Werte/i')

    // At least some field info should be visible after expanding
    await expect(fieldLabels.first()).toBeVisible({ timeout: 5000 }).catch(() => {
      // It's okay if no fields shown - might not have expandable diff
    })
  })

  test("should have undo functionality", async ({ authenticatedPage: page }) => {
    await page.goto("/changelog")
    ...
    await expect(undoButton.first()).toBeVisible({ timeout: 3000 }).catch(() => {
      // Not all entries may have undo button visible
    })
  })

  test("should show undo confirmation dialog", async ({ authenticatedPage: page }) => {
    await page.goto("/changelog")
